...
export default function AdminOfferPaymentsPage() {
  ...
  function handleAdd(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await addPayment(offerId, formData);
      if (result.error) {
  ...
  function handleDelete(id: string) {
    startTransition(async () => {
      const result = await deletePayment(offerId, id);
      if (result.error) {
        toast.error(result.error);
      } else {
              ...
              {payments.map((p) => (
                <div key={p.id} className="flex items-center justify-between rounded-md border p-2">
                  <div className="min-w-0">
                    <p className="text-sm font-medium">{p.household?.name ?? "Unknown"}</p>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-green-600">+${Number(p.amount).toFixed(2)}</span>
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => handleDelete(p.id)} disabled={isPending}>
